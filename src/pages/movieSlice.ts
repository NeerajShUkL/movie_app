...
const initialState: MovieSlice = {
  ...
  page: 1,
  category: "movie",
  totalYear: ['2023', '2022', '2021', '2020', '2019',
...
export const fetchPapularMovies = createAsyncThunk(
  ...
  async ({ page, category }: { page?: Number, category?: string }) => {
    const res: SelectedMovie = await fetch(`https://api.themoviedb.org/3/${category}/popular?api_key=9fd5b693c3907797a39f74f5f46395e8&page=${page}&language=en-US`)
      .then(
        (data) => data.json()
      )
    return res
...
export const fetchSerchedMovies = createAsyncThunk(
  ...
  async ({ searchedValue, category }: { searchedValue?: string, category?: string }) => {
    const res: SelectedMovie = await fetch(`https://api.themoviedb.org/3/search/${category}?api_key=9fd5b693c3907797a39f74f5f46395e8&language=en-US&query=${searchedValue}&page=1&include_adult=false&language=en-US`)
      .then(
        (data) => data.json()
      )
    return res
...
export const fetchSelectedMovie = createAsyncThunk(
  ...
  async ({ id, category }: { id?: Number | undefined, category?: string }) => {
    const res: SelectedMovie = await fetch(`https://api.themoviedb.org/3/${category}/${id}?api_key=9fd5b693c3907797a39f74f5f46395e8&append_to_response=videos&language=en-US`)
      .then(
        (data) => data.json()
      )
    return res
...
export const fetchSelectedYearMovie = createAsyncThunk(
  ...
  async ({ category, selectedYear, page }: { category?: string, selectedYear?: string, page?: Number }) => {
    const releaseDate: string = category === 'movie' ? 'primary_release_date' : 'first_air_date'

    const res: SelectedMovie = await fetch(`https://api.themoviedb.org/3/discover/${category}?api_key=9fd5b693c3907797a39f74f5f46395e8&${releaseDate}.gte=${selectedYear}-01-01&${releaseDate}.lte=${selectedYear}-12-31&page=${page}`)
      .then(
        (data) => data.json()
      )
    return res
...
export const movieSlice = createSlice({
  ...
  reducers: {
    ...
    removeFavorite: (state, action: PayloadAction<Number>) => {
      ...
      state.favorite.splice(state.favorite.findIndex((fid: Number) => fid === action.payload), 1)
    },
    removeFavoriteList: (state, action: PayloadAction<Number>) => {
      // state.favorite = initialState
      state.favoriteList.splice(state.favoriteList.findIndex((fid: any) => fid.id === action.payload), 1)
    },
  },
...
export const { setSearch,
  ...
  setFavoriteList,  
  removeFavorite,
  removeFavoriteList, } = movieSlice.actions;
