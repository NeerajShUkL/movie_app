...
import { AppDispatch, fetchPapularMovies, fetchSelectedYearMovie, fetchSerchedMovies, removeFavorite, RootState, setFavorite, setListOfMovies, setPage, setSelectedYear } from '../store/store';
import MovieCard from './MovieCard';
import Pagination from '@mui/material/Pagination';
import Stack from '@mui/material/Stack';
import { BottomNavigation, CircularProgress, FormControl, InputLabel, MenuItem, Select, SelectChangeEvent, Typography } from '@mui/material';
...
const Home = () => {
  ...
  const searchedValue: string = useSelector((state: RootState) => state.movies.searchedValue)
  ...
  const totalYear: string[] = useSelector((state: RootState) => state.movies.totalYear)
  const selectedYear: string = useSelector((state: RootState) => state.movies.selectedYear)
  ...
  const searchedMovie: Movies = useSelector((state: RootState) => state.movies.searchedMovie)
  const listOfMovies: ListOfMovies = useSelector((state: RootState) => state.movies.listOfMovies)
  ...
  const circularProgress = <Box sx={{ display: 'flex', marginTop: 30, marginLeft: '45%', alignItems: 'center', justifyContent: 'center' }}>
    <CircularProgress />
  </Box>


  const movieCardShow = listOfMovies && listOfMovies?.map((movieData: Movies) => {
    let bool = false
    ...
    return (
      <Grid item xs={4} sm={3} md={2} key={movieData.id}>
        <MovieCard
          ...
          title={category === "movie" ? movieData.original_title : movieData.original_name}
        />
      </Grid>
    )
  })
  ...
  return (
    <Box sx={{ flexGrow: 1, mx: 'auto', marginTop: 10, p: 3 }} >
      <Grid container sx={{ justifyContent: 'space-between', width: '100%', marginBottom: 2 }}>
        <Typography display="inline" text-align='left' variant='h6' sx={{ maxWidth: 300 }}>
          {Category}
        </Typography>
      ...
      <BottomNavigation sx={{ position: 'fixed', bottom: 0, width: 1.0 }}>
        <Stack spacing={2} direction="row" justifyContent="center" sx={{ marginTop: 1.5 }}>
          <Pagination count={maxPage} shape="rounded" onChange={handlePagination} />
        </Stack>
      </BottomNavigation>
    </Box>
  )
